...
WAL.
<!-- Unclear what the two uses of "this" mean >
#### Synopsis
...
PGD message payload function that decodes the consensus payloads to a more human-readable output. Used primarily by the `bdr.global_consensus_journal_details` debug view.
<!-- This description is identical to the one preceding >
...
-   `cascade` &mdash; Deprecated, will be removed in a future release.
...
<! Could this just start with "Used internally" >
