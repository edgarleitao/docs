...
BDR 3.7 node can join a EDB Postgres Distributed cluster running 3.6.x at a specific
...
PostgreSQL (or 2ndQPostgres) 12 or 13 to a EDB Postgres Distributed cluster running 3.6.x on
...
On PostgreSQL, it is important to ensure that slot's sync up has completed on
the standby before promoting it. The following query can be run on the
standby in the target database to monitor and ensure that the slots have
...
It is also possible to nudge the slot sync-up process in the entire BDR
cluster by manually performing WAL switches and by requesting all BDR
peer nodes to replay their progress updates. This activity will cause
the group slot to move ahead in a short timespan, and also hasten the
slot sync-up activity on the standby. The following queries can be run
...
Use the monitoring query from above on the standby to check that these
queries indeed help in faster slot sync-up on that standby.
...
When bdr-enterprise extension is installed, you can can manually ensure the
group slot is advanced on all nodes (as much as possible), which helps hasten
the creaation of BDR-related replication slots on a physical standby using the
...
2.  Inform the EDB Postgres Distributed cluster of the change in address by executing the
...
important to set in a EDB Postgres Distributed cluster where there is a Primary -> Physical 
...
When the Decoding Worker is enabled, BDR stores `Logical Change Record` (LCR, in
...
GUCs control the production and usage of LCR per node. By default
these are `false`.  For production and usage of LCRs we need the
Decoding Worker to be enabled for the BDR group and these GUCs to be
set to `true` on each of the nodes in BDR group.

-   `pglogical.enable_wal_decoder` - when turned `false`, all WAL
    Senders using LCRs are restarted to use WAL directly. When `true`
...
As of now, a Decoding Worker decodes changes corresponding to the node where it is
...
is still vital to the proper operation of the EDB Postgres Distributed cluster. If it is dropped,
...
node may well be re-added under the very same name as a parted node.
...
!!! Warning
    ...
    occur when
    (1) parting a node from a EDB Postgres Distributed cluster, then
...
-   `-S` - Instead of dropping logical replication subscriptions, just disable
    them.
-   `-t, --timeout=TIMEOUT`	- Wait no more than TIMEOUT seconds for join to finish
